  ...
  <v-container>
    <v-stepper v-model="e6" vertical>
      ...
      <v-stepper-step :complete="e6 > 2" step="2"
        >Sign-in to your Google account</v-stepper-step
      >
      ...
      <v-stepper-content step="2">
        ...
        <v-banner v-if="authorized">
          ...
          <template v-slot:actions>
            <v-btn text color="error" @click="handleSignoutClick"
              >Sign Out</v-btn
            >
          </template>
        </v-banner>
      ...
      <v-stepper-content step="4">
        <v-banner
          >Events have been created ! Check your Google Calendar :-)</v-banner
        >
      </v-stepper-content>
    </v-stepper>
  </v-container>
...
export default {
  ...
  data: () => ({
    ...
    columns: [
      ...
      { name: "StartDateTime", value: "StartDateTime" },
        ...
        .then((gapi) => {
          ...
          this.results.forEach((event) => {
            if (event.Session && !event.Hollyday) {
              ...
              if (event.GroupEmail) {
                newEvent.attendees = [{ email: event.GroupEmail }];
              }
